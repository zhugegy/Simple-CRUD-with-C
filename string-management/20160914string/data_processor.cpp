...
static int data_processor_send_error_msg(int nErrorNum, Param *ppParam);
static int data_processor_remove_quoter(char *szTarget);
static int data_processor_add(Param *ppParam);
static int data_processor_is_serial_avaiable(int nNum, Param *ppParam);
...
static int data_processor_delete(Param *ppParam);
...
static int data_processor_list_operation_all(Param *ppParam)
{
  ...
  for (psnTmp = ppParam->psnStringNodes; psnTmp; psnTmp = psnTmp->psnNext)
  {
    sprintf_s(szTmp, MAX_INFO_STRING_LENGTH, "%d: %s 创建时间:%s\r\n", 
      psnTmp->nSerial, ppParam->pchArrayStorage + psnTmp->nArrayLocation, 
